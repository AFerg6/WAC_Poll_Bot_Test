...
def get_verification_settings(guild_id: int) -> tuple[GuildSettings, list[str]]:
    server_settings = guild_settings_cache.get(guild_id)
    if server_settings is None:
        server_settings = GuildSettings(guild_id)
        guild_settings_cache[guild_id] = server_settings

    missing_settings = [
        setting for setting in VERIFICATION_SETTING_KEYS
        if server_settings.get_id(setting, None) is None
    ]
    return server_settings, missing_settings
...
# User verification tables - by luki
cursor.execute(
    """
...
#-------- Lyndons verification system functions --------
# All original methods are copied from lukis identity_verification.py file
...
async def assign_unverified_role(member: discord.Member) -> None:
    server_settings, _missing_settings = get_verification_settings(member.guild.id)
    unverified_role_id = server_settings.get_id("UNVERIFIED_ROLE_ID", None)
...
async def complete_verification(member: discord.Member, submission: str) -> str:
    ...
    server_settings, _missing_settings = get_verification_settings(member.guild.id)
    verified_role_id = server_settings.get_id("VERIFIED_ROLE_ID", None)
    unverified_role_id = server_settings.get_id("UNVERIFIED_ROLE_ID", None)
...
class verification_group(commands.Cog, name='Verification'):
    ...
    # By Luki
    @commands.command(name="verify", brief="Verify your @uwo.ca email")
    async def verify(self, ctx, *, email_or_code: str = ""):
        server_settings, missing_settings = get_verification_settings(ctx.guild.id)
        if missing_settings:
            missing = ", ".join(missing_settings)
            await ctx.send(
                f"Verification is not configured. Missing setting(s): {missing}. "
                "An administrator must configure all three verification settings first.",
            )
            return

        verification_channel_id = server_settings.get_id("VERIFICATION_CHANNEL_ID")
        if ctx.channel.id != verification_channel_id:
            await ctx.send(
                f"Please use <#{verification_channel_id}> for verification.",
            )
            return
        ...
        if re.fullmatch(r"\d{6}", text):
            result = await complete_verification(ctx.author, text)
        else:
            result = await start_verification(ctx.author, text)
